...
function DiscreteUniform() {
	...
	if ( arguments.length ) {
		...
		b = arguments[ 1 ];
		if ( !isInteger( a ) ) {
			throw new TypeError( format( '0yb8s,HF', a ) );
		}
		if ( !isInteger( b ) ) {
			throw new TypeError( format( '0yb8t,HG', b ) );
		}
		if ( a > b ) {
			throw new RangeError( format( '0ybHH', a, b ) );
		}
	} else {
		a = 0;
		b = 1;
	...
	defineProperty( this, 'a', {
		...
		'set': function set( value ) {
			if ( !isInteger( value ) ) {
				throw new TypeError( format( '0yb8v,HI', value ) );
			}
			if ( value > b ) {
				throw new RangeError( format( '0yb8w,HJ', b, value ) );
			}
			a = value;
		}
	});
	defineProperty( this, 'b', {
		...
		'set': function set( value ) {
			...
			if ( a > value ) {
				throw new RangeError( format( '0yb8x,HK', a, value ) );
			}
			b = value;
		}
	});
